...
if [[ ${zinsults_load[(r)taiwanese]} == taiwanese ]];then
	msgs+=(
	"R 嚶嚶嚶嚶嚶嚶嚶嚶"
	"一定把你鼻樑打斷"
	"你打錯指令了但不要緊張～"
	"你是不是一直…為了做效果待在這個地方啊，奇怪…看你這個，看你這個開打的實力，少說也是大師等級的啊～"
	"又錯！又錯！又！又打錯了！"
	"吧嘎 Nono～"
	"好了拉不要作效果打錯字拉！"
	"好啊都這樣啊都打錯指令啊"
	"幹嘛？你以為你的指令可以擋我的二技能是不是？"
	"打錯指令都不揪啊"
	"狠狠的把他甩到旁邊去～"
	"這到底什麼錯字歐齁齁齁齁～"
	"還敢打錯啊？你誰都可以得罪就不要給我得罪這個瘋子拉！"
	"還趕來 gank 指令啊？送頭！"
)
fi

if [[ ${zinsults_load[(r)asian]} == asian ]]; then
	msgs+=(
	"Failure!"
	"Hallo? Hallo? Beijing Embassy! There's a failure who keeps typing irrational stuff!"
	"I will report your incident to jesus"
	"Stoopid!"
	"Type correct commands, or be a failure"
	"Type correct commands, or homeless"
	"Type correct commands, or unemployment"
	"We're Asians, not Bsians"
	"What da hail?"
	"You see that? Stoopid"
)
fi
#
